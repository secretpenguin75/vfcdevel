...
def Profile_gen4(Date, Tatm, Tp, Proxies, rho, Te = None, mixing_level=0, noise_level=0, mixing_scale_m = 40*1e-3, noise_scale_m = 10*1e-3, res = 1e-3,
    ...
    Tmean = np.nanmean(Tatm) # mean atmosphere temperature for Herron and Langway??
    ...
    if keeplog is False and Te is None:
        ...
        D = 19/24/3600/365.25 # temperature diffusivity in firn, 19 meter per year, to seconds
        #dt = np.diff(Date)[0]*365.25*24*3600 # timestep in decimalyear to seconds
        dt = np.diff(Date)[0]/np.timedelta64(1)/1e9 # timestep in numpy.timedelta64 to seconds
        ...
        Ti = np.full((Tdepth).shape,np.nanmean(Tsol))
        ...
        for i in timebar:
            ...
            if i == 0:
                maski = (time_even<=Time.iloc[0]) # layer beetween snow deposited at time t_i and snow deposited at time t_i+1
            elif i==len(Date)-1:
                ...
                maski = (time_even>Time.iloc[i-1])
            else:
                maski = (time_even<=Time.iloc[i])&(time_even>Time.iloc[i-1]) # layer beetween snow deposited at time t_i and snow deposited at time t_i+1    

            
            # IN CASE OF NEW SNOW: SHIFT THE PROFILE, ELSE JUST REUSE PROFILE OF TIMESTEP i-1
            if sum(maski)>0:
            ...
            # END OF TIME LOOP, STORE AWAY STATE AT TIME i and ITERATE
    ...
    # NOW THAT we have an even depth axis, we can apply diffusion that works in snow depth
